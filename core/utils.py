...
def recognize_plate(img, coords,plate_pred,car_id):
    ...
    if xmin < 5 or ymin < 5:
      box = img[int(ymin):int(ymax)+5, int(xmin):int(xmax)+5]
    else:
    ...
    for cnt in sorted_contours:
        ...
        try:
            text = pytesseract.image_to_string(roi, config='-c tessedit_char_whitelist=0123456789ABEKMHOPCTYX --psm 8 --oem 3')
    ...
    if len(plate_num) != 0:
      ...
      for plate in plate_pred[car_id]:
        ...
        for i in range(leng):
          ...
          if plate[i] in maj_vote_counts[i]: maj_vote_counts[i][plate[i]] += 1
          else: maj_vote_counts[i][str(plate[i])] = 1
          # Get most frequent occurence
      for i in range(len(maj_vote_counts)):
        if(maj_vote_counts[i] != {}):
          choice = max(maj_vote_counts[i], key=lambda key: maj_vote_counts[i][key])
          result.append(choice)

      maj_vote_candidate = "".join(result)

      print("majority voting candidate#: ", maj_vote_candidate)
...
def draw_bbox(image, bboxes,timestamp,frameStorage,plate_pred, info = False, counted_classes = None, show_label=True, allowed_classes=list(read_class_names(cfg.YOLO.CLASSES).values()), read_plate = False):
    ...
    for i in range(num_boxes):
        ...
        w_b, h_b = int(coor[2] ), int(coor[3])
        x_b, y_b = int((coor[0]) - w_b / 2), int((coor[1]) - h_b / 2)
        ...
        if class_name not in allowed_classes:
            ...
            if read_plate:
                ...
                plate_number = recognize_plate(image, coor,plate_pred,car_id)
